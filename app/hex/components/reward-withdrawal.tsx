...
import { ArrowDownToLine, Clock, CheckCircle2, XCircle, ExternalLink, Wallet, AlertTriangle, Users } from "lucide-react"
...
import type { WithdrawalHistory, WithdrawalFormData, WithdrawalStatus, UserBalance, UserData } from "@/types"
...
export default function RewardWithdrawal({ userBalance }: RewardWithdrawalProps): JSX.Element {
  const [withdrawalHistory, setWithdrawalHistory] = useState<WithdrawalHistory[]>(sampleWithdrawalHistory)
  ...
  const [savedAddresses, setSavedAddresses] = useState<string[]>(["0x1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b"])
  ...
  const [daysUntilNextWithdrawal, setDaysUntilNextWithdrawal] = useState<number>(0)
  ...
  const handleWithdrawAll = (): void => {
    const maxAmount = formData.currency === "HXCO" ? userBalance.hxco : userBalance.ton
    setFormData({
      ...formData,
      amount: maxAmount,
    })
  ...
  const validateForm = (): boolean => {
    ...
    if (!canWithdraw) {
      setWithdrawalError(`You need to wait ${daysUntilNextWithdrawal} more days before your next withdrawal`)
      return false
    }
    ...
    if (formData.amount < MIN_WITHDRAWAL[formData.currency]) {
      setWithdrawalError(`Minimum withdrawal amount is ${MIN_WITHDRAWAL[formData.currency]} ${formData.currency}`)
      return false
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8">
      ...
      <Card className="lg:col-span-1 border border-cosmic-accent/20 bg-cosmic-card/50 backdrop-blur-sm h-fit">
        <CardHeader className="px-4 sm:px-6">
          <CardTitle className="text-cosmic-text text-lg sm:text-xl">Withdraw Your Funds</CardTitle>
          <CardDescription className="text-cosmic-muted">Transfer your earnings to your wallet</CardDescription>
        </CardHeader>
        <CardContent className="px-4 sm:px-6">
          <form onSubmit={handleWithdrawalSubmit}>
            <div className="space-y-4 sm:space-y-6">
              ...
              {!canWithdraw && (
                <Alert className="bg-cosmic-secondary/10 border-cosmic-secondary/30">
                  ...
                  <AlertTitle className="text-cosmic-secondary text-sm">Withdrawal Cooldown</AlertTitle>
                  <AlertDescription className="text-cosmic-secondary/80 text-xs sm:text-sm">
                    You need to wait {daysUntilNextWithdrawal} more days before your next withdrawal. Last withdrawal
                    was on {lastWithdrawalDate}.
                  </AlertDescription>
                </Alert>
              )}
              ...
              >
                <Users className={`h-4 w-4 ${referralCount >= 10 ? "text-green-500" : "text-cosmic-secondary"}`} />
                <AlertTitle className={`${referralCount >= 10 ? "text-green-500" : "text-cosmic-secondary"} text-sm`}>
                  Referral Status
                </AlertTitle>
                <AlertDescription
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={formData.currency} onValueChange={handleCurrencyChange} className="flex space-x-4">
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="HXCO" id="hxco" className="border-cosmic-accent text-cosmic-accent" />
                    <Label
                      htmlFor="hxco"
                  ...
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="TON" id="ton" className="border-cosmic-tertiary text-cosmic-tertiary" />
                    <Label
                      htmlFor="ton"
                    ...
                    >
                      TON{" "}
                      {referralCount < 10 && (
                        <span className="text-xs text-cosmic-secondary">(10 referrals needed)</span>
                      )}
                    </Label>
                  </div>
                </RadioGroup>
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="text-xs text-cosmic-muted">
                    Available: {formData.currency === "HXCO" ? userBalance.hxco : userBalance.ton.toFixed(2)}{" "}
                    {formData.currency}
                  </span>
                </div>
                <div className="relative">
                  ...
                  <div className="absolute inset-y-0 right-0 flex items-center pr-3">
                    <span className="text-cosmic-muted">{formData.currency}</span>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                {savedAddresses.length > 0 && (
                  <div className="mt-2">
                    <Label className="text-xs text-cosmic-muted mb-1 block">Saved Addresses</Label>
                    <div className="flex flex-wrap gap-2">
                      {savedAddresses.map((address, index) => (
                        <button
                          ...
                          onClick={() => canWithdraw && setFormData({ ...formData, address })}
                          disabled={!canWithdraw}
                        >
              ...
              {/* Error Message */}
              {withdrawalError && (
                <Alert variant="destructive" className="border-cosmic-secondary/30 bg-cosmic-secondary/10">
                  <AlertTriangle className="h-4 w-4 text-cosmic-secondary" />
                  <AlertTitle className="text-cosmic-secondary text-sm">Error</AlertTitle>
                  <AlertDescription className="text-cosmic-secondary/80 text-xs sm:text-sm">
                    {withdrawalError}
                  </AlertDescription>
                </Alert>
            ...
            <Button
              ...
              disabled={!canWithdraw}
            >
              {!canWithdraw ? `Wait ${daysUntilNextWithdrawal} Days` : "Withdraw Funds"}
            </Button>
          </form>
        </CardContent>
      </Card>
      ...
      <Card className="lg:col-span-2 border border-cosmic-accent/20 bg-cosmic-card/50 backdrop-blur-sm">
        <CardHeader className="px-4 sm:px-6">
          <CardTitle className="text-cosmic-text text-lg sm:text-xl">Withdrawal History</CardTitle>
                  ...
                  <TableBody>
                    {withdrawalHistory.map((withdrawal) => (
                      <TableRow key={withdrawal.id}>
                        <TableCell className="text-cosmic-muted text-xs sm:text-sm">{withdrawal.date}</TableCell>
                        <TableCell
                          className={`font-medium text-xs sm:text-sm ${
                            withdrawal.currency === "HXCO" ? "text-cosmic-accent" : "text-cosmic-tertiary"
                          }`}
                        >
      ...
      <Card className="lg:hidden border border-cosmic-accent/20 bg-cosmic-card/50 backdrop-blur-sm h-fit">
        <CardHeader className="px-4 sm:px-6">
          <CardTitle className="text-cosmic-text text-lg">Withdrawal Info</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 px-4 sm:px-6">
          <div>
            <h3 className="text-sm font-medium text-cosmic-text mb-1">Processing Time</h3>
            <p className="text-xs text-cosmic-muted">Withdrawals are typically processed within 24 hours.</p>
          </div>
          <div>
            <h3 className="text-sm font-medium text-cosmic-text mb-1">Minimum Withdrawal</h3>
            <div className="space-y-1">
              <p className="text-xs flex justify-between">
                <span className="text-cosmic-muted">HXCO</span>
                <span className="text-cosmic-accent">{MIN_WITHDRAWAL.HXCO} HXCO</span>
              </p>
              <p className="text-xs flex justify-between">
                <span className="text-cosmic-muted">TON</span>
                <span className="text-cosmic-tertiary">{MIN_WITHDRAWAL.TON} TON</span>
              </p>
            </div>
          </div>
          <div>
            <h3 className="text-sm font-medium text-cosmic-text mb-1">Withdrawal Cooldown</h3>
            <p className="text-xs text-cosmic-muted">
              You can only make one withdrawal every 15 days. This helps us manage processing fees and ensure security.
            </p>
          </div>
          <Alert className="bg-cosmic-accent/5 border-cosmic-accent/20">
            <Wallet className="h-4 w-4 text-cosmic-accent" />
            <AlertTitle className="text-cosmic-text text-sm">Double-check your address</AlertTitle>
            <AlertDescription className="text-cosmic-muted text-xs">
              Make sure you enter the correct wallet address. Transactions cannot be reversed.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
      ...
      <Card className="hidden lg:block lg:col-span-1 border border-cosmic-accent/20 bg-cosmic-card/50 backdrop-blur-sm h-fit">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <h3 className="text-sm font-medium text-cosmic-text mb-1">TON Referral Bonus</h3>
            <p className="text-xs text-cosmic-muted">
              Invite friends to earn 0.01 TON per referral (up to 10 referrals). You need at least 10 referrals to
              withdraw TON.
            </p>
          </div>
          <div>
            ...
            <p className="text-xs text-cosmic-muted">
              No withdrawal fees are charged by our platform. Network fees may apply.
            </p>
          </div>
          <Alert className="bg-cosmic-accent/5 border-cosmic-accent/20">
            <Wallet className="h-4 w-4 text-cosmic-accent" />
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent className="shadow-xl max-w-sm mx-auto bg-cosmic-card border-cosmic-accent/20">
          <DialogHeader>
            ...
            <DialogDescription className="text-cosmic-muted">Please review your withdrawal details</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-cosmic-text text-xs">{formatAddress(formData.address)}</span>
            </div>
            <Alert className="bg-cosmic-accent/5 border-cosmic-accent/20">
        ...
        <DialogContent className="shadow-xl max-w-sm mx-auto bg-cosmic-card border-cosmic-accent/20">
          <div className="flex flex-col items-center justify-center py-6">
            ...
            <DialogDescription className="text-cosmic-muted text-center max-w-xs">
              Your withdrawal request has been submitted and is being processed. You can track its status in the
              withdrawal history.
            </DialogDescription>
          </div>
          <DialogFooter>
